...
RUNTIME_DIAGNOSTIC_ISSUE_KIND_PATTERN = (
    "^(" + "|".join(sorted(RUNTIME_DIAGNOSTIC_ISSUE_KINDS)) + ")$"
)
RUNTIME_BACKLOG_SCOPE_KINDS = frozenset({"site_id", "ability_family", "execution_pattern"})
RUNTIME_BACKLOG_SCOPE_KIND_PATTERN = "^(" + "|".join(sorted(RUNTIME_BACKLOG_SCOPE_KINDS)) + ")$"
...
RUNTIME_BACKLOG_QUEUED_AGING_AFTER_SECONDS = RUNTIME_DIAGNOSTIC_QUEUED_STALE_AFTER_SECONDS // 2
RUNTIME_BACKLOG_RUNNING_AGING_AFTER_SECONDS = RUNTIME_DIAGNOSTIC_RUNNING_STALE_AFTER_SECONDS // 2
RUNTIME_CALLBACK_DISPATCH_LEASE_RECOVERY_AFTER_SECONDS = (
    RUNTIME_DIAGNOSTIC_CALLBACK_DISPATCHING_STALE_AFTER_SECONDS
)
RUNTIME_CALLBACK_DISPATCH_LEASE_RECOVERY_ERROR_CODE = "runtime.callback_dispatch_lease_recovered"
...
def normalize_runtime_task_backend(task_backend: dict[str, Any] | None) -> dict[str, object]:
    ...
    return {
        key: value for key, value in normalized.items() if key in ALLOWED_RUNTIME_TASK_BACKEND_KEYS
    }
