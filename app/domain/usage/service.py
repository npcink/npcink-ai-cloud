...
class UsageService:
    ...
    def get_prompt_advisor_recommendation_summary(
        self,
        *,
        site_id: str,
        filters: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        normalized_filters = self._normalize_prompt_advisor_filters(filters)
        logs_filters = {
            "range": normalized_filters["range"],
            "start_gmt": normalized_filters["start_gmt"],
            "end_gmt": normalized_filters["end_gmt"],
            "status": "all",
            "limit": 25,
            "ability_id": normalized_filters["ability_id"],
        }
        logs_summary = self.get_logs_analytics_summary(site_id=site_id, filters=logs_filters)
        recommendations = self.get_logs_analytics_recommendations(
            site_id=site_id,
        ...
        current_variant = normalized_filters["current_variant"] or "current"
        candidate_variant = normalized_filters["candidate_variant"] or "candidate"
        current_version = normalized_filters["current_version"]
        candidate_version = normalized_filters["candidate_version"]
        ...
        recommended_error_codes = [
            str(code).strip()
            for code in recommendations.get("recommended_error_codes", [])
            if str(code).strip()
        ]
        ...
        if blocked_rate >= 0.1:
            risk_lines.append(
                "Blocked call rate is elevated in the recent window "
        ...
        recommend_candidate = (
            ...
            and len(degraded_provider_ids) == 0
            and error_rate < 0.2
            and blocked_rate < 0.1
        )
        if recommend_candidate:
            recommended_variant = candidate_variant
            recommended_version = candidate_version
        elif current_version > 0:
            recommended_variant = current_variant
        ...
        scope_label_parts = [
            value
            for value in (
                normalized_filters["ability_id"],
                normalized_filters["prompt_id"],
            )
            if value
        ]
        ...
        if recommend_candidate:
            reason_lines.append(
                "Cloud runtime signals are stable enough for a local manual preview/adopt decision."
            )
        elif current_version > 0:
            reason_lines.append(
                "Keep the current local baseline until the service-plane risk signals settle or the candidate is re-evaluated."
            )
        else:
            reason_lines.append(
                "This summary is advisory only; cloud does not own prompt truth or publish state."
            )
        ...
        if scope_label:
            ...
            reason_lines.append(
                "No ability or prompt scope was provided; this uses site-level service-plane evidence."
            )
        if candidate_version > 0 and current_version > 0 and candidate_version < current_version:
            reason_lines.append(
        ...
        if not updated_at:
            updated_at = self._format_datetime(self.now_factory())

        return {
            "source": "cloud_prompt_advisor_summary",
            "site_id": site_id,
            "ability_id": normalized_filters["ability_id"],
            "prompt_id": normalized_filters["prompt_id"],
            ...
            "risk_summary": " ".join(risk_lines).strip(),
            ...
            "evidence": {
                "logs_summary": {
                    "total": int(logs_summary.get("total") or 0),
                    "success_rate": float(logs_summary.get("success_rate") or 0.0),
            ...
            "boundary_note": (
                "Readonly service-plane recommendation only. Local plugin remains the "
                "prompt truth owner and final adoption owner."
            ),
        }

    def get_prompt_eval_recommendation_summary(
        self,
        *,
        site_id: str,
        filters: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        return self._build_prompt_governance_recommendation_summary(
            site_id=site_id,
            filters=filters,
            recommendation_type="eval",
        )

    def get_prompt_canary_recommendation_summary(
        self,
        *,
        site_id: str,
        filters: dict[str, Any] | None = None,
        ...
        return self._build_prompt_governance_recommendation_summary(
            site_id=site_id,
            filters=filters,
            recommendation_type="canary",
        )

    def get_prompt_upgrade_recommendation_summary(
        self,
        *,
        site_id: str,
        filters: dict[str, Any] | None = None,
    ...
    def get_preset_advisor_recommendation_summary(
        self,
        *,
        site_id: str,
        filters: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        normalized_filters = self._normalize_preset_advisor_filters(filters)
        logs_filters = {
            "range": normalized_filters["range"],
            "start_gmt": normalized_filters["start_gmt"],
            "end_gmt": normalized_filters["end_gmt"],
        ...
        context = self._build_logs_analytics_context(site_id=site_id, filters=logs_filters)

        selected_preset_id = normalized_filters["preset_id"]
        default_preset_id = normalized_filters["default_preset_id"]

        preset_rows_by_id: dict[str, dict[str, Any]] = {}
        for provider_call in context["provider_calls"]:
            ...
            row = preset_rows_by_id.get(preset_id)
            if row is None:
                row = {
                    "preset_id": preset_id,
                    "runs_total": 0,
                ...
                preset_rows_by_id[preset_id] = row

            row["runs_total"] += 1
            if str(run.status or "").strip().lower() == "succeeded":
                row["success_total"] += 1
            if bool(run.fallback_used):
                row["fallback_total"] += 1

            created_at = self._normalize_datetime(provider_call.created_at)
            latest_seen_at = str(row.get("latest_seen_at") or "").strip()
            created_at_display = self._format_datetime_or_empty(created_at)
            if created_at_display and created_at_display > latest_seen_at:
                row["latest_seen_at"] = created_at_display
            ...
            (
                {
                    **row,
                    "success_rate": self._safe_rate(
                        int(row["success_total"]),
                        int(row["runs_total"]),
                    ),
                    "fallback_rate": self._safe_rate(
                        int(row["fallback_total"]),
                        int(row["runs_total"]),
                    ),
                }
                for row in preset_rows_by_id.values()
            ),
            key=lambda item: (
                -int(item["runs_total"]),
        ...
        observed_top = observed_rows[0] if observed_rows else {}
        observed_top_preset_id = str(observed_top.get("preset_id") or "").strip()
        recommended_providers = [
            str(provider).strip()
            for provider in recommendations.get("recommended_providers", [])
        ...
        if observed_top_preset_id:
            recommended_preset = observed_top_preset_id
        elif selected_preset_id:
            recommended_preset = selected_preset_id
        else:
            recommended_preset = default_preset_id

        reason_lines: list[str] = []
        if observed_top_preset_id:
            reason_lines.append(
                "Cloud runtime evidence currently favors preset "
                + observed_top_preset_id
        ...
        if selected_preset_id and recommended_preset == selected_preset_id:
            reason_lines.append("The current local preset remains the preferred baseline.")
        elif selected_preset_id and recommended_preset:
            reason_lines.append(
                "Keep final adoption local; cloud only suggests whether "
                + recommended_preset
                + " deserves review."
            )

        recommended_changes: list[str] = []
        if recommended_preset and selected_preset_id and recommended_preset != selected_preset_id:
            recommended_changes.append(
                "Review whether local preset selection should move from "
                + selected_preset_id
                + " to "
                + recommended_preset
                + "."
            ...
            recommended_changes.append(
                "Keep "
                + recommended_preset
                + " as the local baseline unless manual preview or operator review says otherwise."
            )
        if recommended_providers:
            recommended_changes.append(
                "Check local provider/model defaults against recommended providers: "
                + ", ".join(recommended_providers[:5])
                + "."
            )
        if degraded_provider_ids or error_rate >= 0.2 or blocked_rate >= 0.1:
            recommended_changes.append(
                "Hold local adoption to manual review while degraded providers or elevated runtime risk remain."
            )
        if not recommended_changes:
            recommended_changes.append(
                "Collect more runtime evidence before changing local preset truth."
            )
        ...
        if observed_top_preset_id:
            impact_lines.append(
                observed_top_preset_id
                + " handled "
                + str(int(observed_top.get("runs_total") or 0))
                + " recent provider calls with success rate "
                + str(round(float(observed_top.get("success_rate") or 0.0) * 100, 2))
                + "%."
            )
        if recommended_providers:
            impact_lines.append(
                "Recommended providers in the current window: "
                + ", ".join(recommended_providers[:5])
                + "."
            )
        if degraded_provider_ids:
            impact_lines.append(
                "Degraded providers may reduce preset effectiveness: "
                + ", ".join(degraded_provider_ids[:5])
                + "."
            )
        if not impact_lines:
            impact_lines.append(
                "This is a readonly cloud recommendation summary, not a preset apply action."
            )
        ...
        return {
            "source": "cloud_preset_advisor_summary",
            "site_id": site_id,
            "ability_id": normalized_filters["ability_id"],
            ...
            "reason_summary": " ".join(reason_lines).strip(),
            "impact_summary": " ".join(impact_lines).strip(),
            "updated_at": updated_at,
            "window": {
                "range": normalized_filters["range"],
                "start_gmt": normalized_filters["start_gmt"],
            ...
            "evidence": {
                ...
                "observed_presets": observed_rows[:5],
            ...
            "boundary_note": (
                "Readonly service-plane recommendation only. Local plugin remains the "
                "preset truth owner and final adoption owner."
            ),
        }
    ...
    def _build_prompt_governance_recommendation_summary(
        self,
        *,
        site_id: str,
        ...
        return {
            "source": "cloud_prompt_governance_recommendation_summary",
            "recommendation_type": normalized_type or "upgrade",
            "site_id": site_id,
            "ability_id": normalized_filters["ability_id"],
            "prompt_id": normalized_filters["prompt_id"],
            "current_variant": current_variant,
    ...
    def _normalize_prompt_advisor_filters(
        self,
        filters: dict[str, Any] | None,
    ) -> dict[str, Any]:
        base_filters = filters.copy() if isinstance(filters, dict) else {}
        ...
        return {
            ...
            "prompt_id": str(base_filters.get("prompt_id") or "").strip()[:191],
            "current_variant": self._normalize_prompt_advisor_variant(
                base_filters.get("current_variant"),
                "current",
            ),
            "candidate_variant": self._normalize_prompt_advisor_variant(
            ...
            "end_gmt": self._format_datetime(window["end_at"]),
        }

    def _normalize_preset_advisor_filters(
        self,
        filters: dict[str, Any] | None,
    ) -> dict[str, Any]:
        base_filters = filters.copy() if isinstance(filters, dict) else {}
    ...
    def _normalize_prompt_advisor_variant(
        self,
        value: Any,
        default: str,
    ) -> str:
