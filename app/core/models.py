...
class CatalogModelAnnotation(Base):
    __tablename__ = "catalog_model_annotations"

    model_id: Mapped[str] = mapped_column(
        ForeignKey("catalog_models.model_id"),
        primary_key=True,
    )
    ...
    cost_tier: Mapped[str | None] = mapped_column(String(32), index=True)
    visibility: Mapped[str] = mapped_column(String(32), default="default", index=True)
    badges_json: Mapped[list[str]] = mapped_column(JSON, default=list)
...
class RecognitionModelAnnotation(Base):
    __tablename__ = "recognition_model_annotations"
    __table_args__ = (
        UniqueConstraint(
            "provider_id",
            "model_id",
    ...
    manual_tags_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    operator_notes: Mapped[str | None] = mapped_column(Text)
    metadata_json: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
...
class RecognitionSourceRun(Base):
    __tablename__ = "recognition_source_runs"
    __table_args__ = (
        UniqueConstraint("run_id", name="uq_recognition_source_runs_run_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    run_id: Mapped[str] = mapped_column(String(191), index=True)
    source_name: Mapped[str] = mapped_column(String(64), index=True)
    snapshot_generated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), index=True)
...
class RecognitionSnapshotPublication(Base):
    __tablename__ = "recognition_snapshot_publications"
    __table_args__ = (
        UniqueConstraint("revision", name="uq_recognition_snapshot_publications_revision"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    revision: Mapped[str] = mapped_column(String(191), index=True)
    ...
    source_keys_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    source_run_ids_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    record_keys_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    metadata_json: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        server_default=func.now(),
...
class ProviderConnection(Base):
    ...
    source_role: Mapped[str] = mapped_column(String(32), default="execution_source", index=True)
