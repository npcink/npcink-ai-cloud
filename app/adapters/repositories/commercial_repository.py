...
class CommercialRepository(CommercialAccountQueries, CommercialSiteQueries):
    ...
    def query_admin_portal_user_directory_page(
        ...
        *,
        ...
        tier_alias_cases: list[tuple[Any, str]] = [
            (subscription_tier_id == tier_id, alias)
            for tier_id, alias in tier_package_aliases
        ]
        tier_alias_cases.extend(
            (normalized_plan_id.contains(tier_id), alias)
            for tier_id, alias in tier_package_aliases
        )
        ...
        if qq_bound is not None:
            statement = statement.where(
                qq_binding_count > 0 if qq_bound else qq_binding_count == 0
            )
