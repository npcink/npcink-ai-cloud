...
def _build_client(tmp_path: Path) -> tuple[str, TestClient, dict[str, int]]:
    ...
    calls = {"put": 0, "post": 0, "patch": 0, "delete": 0, "get": 0, "head": 0}
    ...
    @app.patch("/internal/test-resource")
    async def patch_resource(request: Request) -> Any:
        auth_error = await authorize_internal_request(
            request,
            require_idempotency=True,
        )
    ...
    @app.delete("/internal/test-resource")
    async def delete_resource(request: Request) -> Any:
        auth_error = await authorize_internal_request(
            request,
            require_idempotency=True,
        )
...
def test_internal_patch_replay_is_blocked_before_handler_runs_twice(tmp_path: Path) -> None:
    database_url, client, calls = _build_client(tmp_path)
    headers = _headers(idempotency_key="internal-patch-replay-001")
    try:
        first = client.patch("/internal/test-resource", json={"enabled": True}, headers=headers)
        replay = client.patch(
            "/internal/test-resource",
            json={"enabled": True},
            headers=headers,
...
def test_internal_delete_replay_is_blocked_before_handler_runs_twice(tmp_path: Path) -> None:
    database_url, client, calls = _build_client(tmp_path)
    headers = _headers(idempotency_key="internal-delete-replay-001")
    try:
        first = client.delete("/internal/test-resource", headers=headers)
        replay = client.delete("/internal/test-resource", headers=headers)
