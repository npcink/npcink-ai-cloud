...
from app.dev.live_site_runtime_resolve_execute_packet import build_execute_packet
from app.dev.live_site_runtime_smoke import APPROVAL_TEXT
...
def _paths(tmp_path: Path) -> dict[str, Path]:
    return {
        ...
        "resolve_prepare_report_path": tmp_path / "resolve" / "runtime-resolve-smoke-report.json",
        "status_report_path": tmp_path / "status" / "trial-status-report.json",
        "approval_file": tmp_path / "resolve-approval.txt",
    }
...
def _write_ready_inputs(paths: dict[str, Path]) -> None:
    ...
    _write_json(
        paths["resolve_prepare_report_path"],
        {
            "mode": "prepare",
            "ok": False,
            "acceptance_failures": [],
...
def test_execute_packet_ready_from_acceptance_prepare_and_status(tmp_path: Path) -> None:
    ...
    assert report["ready_for_runtime_resolve_execute_after_exact_approval"] is True
    assert report["approval"]["required_text"] == APPROVAL_TEXT  # type: ignore[index]
    assert report["commands"]["runtime_resolve_execute"]["argv"] == [  # type: ignore[index]
        ...
        "http://127.0.0.1:8010",
        "--output-dir",
        ".tmp/live-site-runtime-smoke/npcink-resolve",
    ]
    assert "secret_live" not in encoded
    assert "mak1_fake" not in encoded
    assert (tmp_path / "packet" / "runtime-resolve-execute-packet.json").exists()
    assert (tmp_path / "packet" / "summary.md").exists()


def test_execute_packet_blocks_when_acceptance_missing(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    _write_ready_inputs(paths)
    paths["acceptance_report_path"].unlink()

    report = build_execute_packet(**paths, output_dir=tmp_path / "packet")

    assert report["ready_for_runtime_resolve_execute_after_exact_approval"] is False
    assert "acceptance report missing" in report["failures"]


def test_execute_packet_blocks_when_resolve_prepare_path_is_wrong(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    _write_ready_inputs(paths)
    prepare = json.loads(paths["resolve_prepare_report_path"].read_text())
    prepare["request_plan"]["path"] = "/v1/runtime/execute"
    paths["resolve_prepare_report_path"].write_text(json.dumps(prepare) + "\n")

    report = build_execute_packet(**paths, output_dir=tmp_path / "packet")

    assert report["ready_for_runtime_resolve_execute_after_exact_approval"] is False
    assert (
        "resolve prepare request_plan.path expected '/v1/runtime/resolve', "
        "got '/v1/runtime/execute'"
    ) in report["failures"]


def test_execute_packet_blocks_when_status_next_action_is_not_resolve(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    _write_ready_inputs(paths)
    status = json.loads(paths["status_report_path"].read_text())
    status["next_action"]["phase"] = "runtime_execute_smoke"
    status["next_action"]["action"] = "execute_runtime_execute_smoke_after_exact_approval"
    ...
    assert report["ready_for_runtime_resolve_execute_after_exact_approval"] is False
    assert (
        "status next phase expected runtime_resolve_smoke, got 'runtime_execute_smoke'"
        in report["failures"]
    )
