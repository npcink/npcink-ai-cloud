...
require_marker "deploy/deploy-to-ssh-host.sh" '--finalized-runtime-network-repair'
require_marker "deploy/deploy-to-ssh-host.sh" 'Approved for finalized runtime-network repair by operator.'
require_marker ".github/workflows/deploy-production.yml" 'finalized_runtime_network_repair:'
require_marker ".github/workflows/deploy-production.yml" 'Approved for finalized runtime-network repair by operator.'
...
require_marker "deploy/deploy-to-ssh-host.sh" 'Exact ${RELEASE_PLAN_LANE} release plan preserves the running PostgreSQL and Redis services.'
require_marker "deploy/deploy-to-ssh-host.sh" 'Exact ${RELEASE_PLAN_LANE} release plan does not require a database migration.'
require_marker "deploy/deploy-to-ssh-host.sh" 'PLAN_PRESERVE_BACKEND=0'
require_marker "deploy/deploy-to-ssh-host.sh" 'assert_preserved_backend_services_running'
require_marker "deploy/deploy-to-ssh-host.sh" 'Frontend-only release preserves backend provider projections.'
require_marker "docs/cloud-production-release-policy-v1.md" 'An exact `frontend` plan preserves the running API'
