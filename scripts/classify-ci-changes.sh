...
for changed_file in "$@"; do
	case "${changed_file}" in
		.github/workflows/ci.yml|.github/workflows/deploy-production.yml|Dockerfile*|*/Dockerfile*|docker-compose*.yml|deploy/image-lock/*|scripts/check-authoritative-cve-ranges.py|scripts/production-image-supply.py|scripts/production-release-preflight.py|scripts/verify-release-bundle-manifest.py|tests/contract/test_container_image_supply_contract.py|tests/contract/test_exact_release_bundle_contract.py|tests/contract/test_production_release_preflight_contract.py)
			authoritative_cve_required=true
			;;
	esac
