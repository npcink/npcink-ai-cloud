...
test('usage counts, period filters and source failure stay honest', async ({ page }, testInfo) => {
  ...
  await page.setViewportSize({ width: 1440, height: 1050 });
  await expect(page.getByRole('link', { name: /Site diagnostics|站点诊断/ })).toHaveAttribute('href', '/admin/troubleshooting?window=168&from=usage-statistics&site=site-a');
  ...
  await expect(page.getByRole('link', { name: /Function diagnostics|功能诊断/ })).toHaveAttribute('href', '/admin/troubleshooting?window=168&from=usage-statistics&function=wp-ai.classification');
  ...
  await expect(observationTabs.getByRole('link', { name: /Media observability|媒体观测/ })).toHaveAttribute('href', '/admin/media-observability?window=168');
  ...
  await page.getByRole('button', { name: /^3 days$|^3 天$/ }).click();
