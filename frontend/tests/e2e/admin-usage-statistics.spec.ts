...
test('usage counts, period filters and source failure stay honest', async ({ page }, testInfo) => {
  ...
  await page.getByText(/View totals and trends|查看统计与趋势/).click();
  await expect(page.getByText(/Grouped by Cloud receipt time|按 Cloud 收到上报的时间统计/)).toBeVisible();
  const editorialSection = page.locator('[data-ui="usage-editorial-outcomes"]');
  await expect(editorialSection.getByRole('button', { name: /View editorial outcomes|查看编辑效果/ })).toHaveAttribute('aria-expanded', 'false');
  await editorialSection.getByRole('button', { name: /View editorial outcomes|查看编辑效果/ }).click();
  ...
  await expect(page.getByRole('link', { name: /Plugin records|插件记录/ })).toHaveAttribute('href', '/admin/plugin-observability?window=168&plugin=test-plugin&from=usage-statistics');
