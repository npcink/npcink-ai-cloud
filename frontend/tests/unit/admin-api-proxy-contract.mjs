...
assert.match(
  source,
  /methods: \['PATCH'\],[\s\S]*?pattern: \/\^service-settings\\\/\(\?:portal-public\|qq-login\|email\|alipay-payment\|site-relink-policy\|accounting-fx\|platform-preferences\|media-recognition-policy\)\$\/[\s\S]*?methods: \['POST'\],[\s\S]*?pattern: \/\^service-settings\\\/\(\?:qq-login\\\/test\|email\\\/test\|email\\\/preview\|alipay-payment\\\/test\)\$\//,
