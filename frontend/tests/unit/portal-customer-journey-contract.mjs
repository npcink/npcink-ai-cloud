...
assert.match(journey, /const deliveredBeforeDeadline = await Promise\.race/);
assert.match(journey, /if \(!deliveredBeforeDeadline\)/);
assert.match(journey, /void request\.then\(markSent\)\.catch/);
