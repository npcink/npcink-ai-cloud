...
import {
  ...
  useBatchDisablePortalUsers,
  useDisablePortalUser,
  usePortalUserAudit,
  usePortalUsersDirectory,
} from './queries';
import type {
  PortalUserFilters,
  PortalUserItem,
...
export function PortalUsersWorkspace() {
  const { t } = useLocale();
  ...
  const appliedFilters = useMemo<PortalUserFilters>(() => ({
    q: appliedQuery,
  ...
  const sort = normalizePortalUserSort(searchParams.get('sort'));
  const offset = normalizePortalUserOffset(searchParams.get('offset'));
  const focusedPrincipalId = searchParams.get('focus') || '';
  const [draftFilters, setDraftFilters] = useState<PortalUserFilters>(appliedFilters);
  ...
  const directoryQuery = usePortalUsersDirectory(requestKey);
  const auditQuery = usePortalUserAudit(auditUser?.principal_id || '');
  const disableMutation = useDisablePortalUser();
  const batchDisableMutation = useBatchDisablePortalUsers();
  const fallbackDirectoryData = directoryQuery.isError
    ? getLatestPortalUsersDirectoryData(queryClient)
    : undefined;
  const directoryData =
    directoryQuery.data || fallbackDirectoryData;
  const users = useMemo(
    () =>
      Array.isArray(directoryData?.items)
        ? directoryData.items
        : [],
    [directoryData]
  );
  const summary = directoryData?.summary || {};
  const total = Number(directoryData?.total || 0);
  ...
  const error = directoryQuery.error
    ? resolveUiErrorMessage(
        ...
        t('admin.portal_users.load_failed', {}, 'Failed to load Portal users.')
      )
    : null;
  const hasLoaded = Boolean(directoryData);
  const loadedAt = directoryData?.loadedAt
    ? new Date(directoryData.loadedAt)
    : null;
  const loadedRequestKey = directoryData?.requestKey || '';
  const displayScope = portalUsersDisplayScope({
    currentRequestKey: requestKey,
    displayedRequestKey: loadedRequestKey,
    isPlaceholderData: directoryQuery.isPlaceholderData,
    hasError: directoryQuery.isError,
  });
  const auditDetail = auditQuery.data || null;
  const auditLoading = auditQuery.isPending && Boolean(auditUser);
  const auditError = auditQuery.error
    ? resolveUiErrorMessage(
        auditQuery.error,
        t('admin.portal_users.audit_load_failed', {}, 'Failed to load user audit.')
      )
    : null;
  const savingPrincipalId = disableMutation.isPending
    ? disableMutation.variables?.principalId || null
    : null;
  const batchSaving = batchDisableMutation.isPending;
  ...
  useEffect(() => {
    setSelectedPrincipalIds((current) => {
      const next = filterActivePortalUserSelection(current, users);
  ...
  const activePrincipalIds = useMemo(
    () => activePortalUserIds(sortedUsers),
    [sortedUsers]
  );
  const activeUsers = sortedUsers.filter((user) =>
    activePrincipalIds.has(user.principal_id)
  );
  const selectedActiveUsers = sortedUsers.filter(
    (user) =>
      activePrincipalIds.has(user.principal_id) &&
      selectedPrincipalIds.includes(user.principal_id)
  );
  ...
  const toggleUserSelection = (principalId: string) => {
    if (destructiveActionsDisabled || !activePrincipalIds.has(principalId)) {
      return;
    }
    setSelectedPrincipalIds((current) => {
      const next = current.includes(principalId)
        ? current.filter((item) => item !== principalId)
        : [...current, principalId];
      return filterActivePortalUserSelection(next, sortedUsers);
    });
  };
  ...
  const loadAuditDetail = (user: PortalUserItem) => {
    setAuditUser(user);
  };
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div role="alert" className="max-w-md text-center">
          ...
          <button type="button" className="btn btn-primary" onClick={() => void directoryQuery.refetch()}>{t('common.retry')}</button>
        </div>
      </div>
    );
  }
  ...
  const isShowingRetainedResults = displayScope.isRetainedScope;
  ...
  return (
    <BackofficePageStack className="space-y-5">
      <BackofficeLayer
        ...
        actions={(
          <>
            <button type="button" className="btn btn-secondary" disabled={refreshing} onClick={() => void directoryQuery.refetch()}>{refreshing ? t('common.loading', {}, 'Loading...') : t('admin.portal_users.refresh_action', {}, 'Refresh users')}</button>
      ...
      {isShowingRetainedResults && !error ? (
        <div
          role="status"
          className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/25 dark:text-amber-200"
        >
          {t('admin.portal_users.retained_notice', {}, 'Showing the last successfully loaded page; it may not match the current filters.')}
        </div>
      ) : null}
          ...
          <div className="space-y-4 border-b border-slate-200/80 px-5 py-5 dark:border-slate-800 md:px-6">
            ...
            <div className="flex flex-wrap items-center gap-2" aria-label={t('admin.portal_users.status_filter_label', {}, 'User status')}>
              ...
              <label className="ml-auto flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
                <input type="checkbox" checked={allActiveSelected} disabled={destructiveActionsDisabled || activeUsers.length === 0} onChange={toggleAllActiveUsers} aria-label={t('admin.portal_users.select_all_active', {}, 'Select all active users on this page')} />
              ...
              {sortedUsers.map((user) => {
                ...
                return (
                  <article key={user.principal_id} role="listitem" data-ui="portal-user-directory-item" className={cn('grid gap-4 border-b border-slate-200/80 px-5 py-5 transition last:border-b-0 dark:border-slate-800 md:grid-cols-[auto_minmax(11rem,0.9fr)_minmax(13rem,1.1fr)] md:items-center md:px-6 2xl:grid-cols-[auto_minmax(12rem,1fr)_minmax(14rem,1.2fr)_minmax(9rem,0.75fr)_auto]', selected ? 'bg-blue-50/65 dark:bg-blue-950/15' : 'hover:bg-slate-50/70 dark:hover:bg-slate-950/35')}>
                    <input type="checkbox" checked={activePrincipalIds.has(user.principal_id) && selectedPrincipalIds.includes(user.principal_id)} disabled={destructiveActionsDisabled || !activePrincipalIds.has(user.principal_id)} onChange={() => toggleUserSelection(user.principal_id)} aria-label={t('admin.portal_users.select_user', { user: user.email || user.principal_id }, 'Select {{user}}')} />
          ...
          footer={
            <>
              ...
              <button
                ...
                disabled={destructiveActionsDisabled || !batchDisableReason.trim() || selectedActiveUsers.length === 0}
                onClick={() => {
