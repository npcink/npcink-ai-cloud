...
export default function UsageStatisticsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true); setRuntime(null); setPlugins(null); setErrors([]); setActivityPage(1);
    void Promise.allSettled([
      ...
      client.request<Plugins>(`/api/admin/plugin-observability?window_hours=${hours}&record_scope=${recordScope}${siteFilter ? `&site_id=${encodeURIComponent(siteFilter)}` : ''}`, { signal: controller.signal }),
  ...
  }, [hours, refresh, recordScope, siteFilter, functionFilter, qualityView]);
  ...
  const activityPageGroups = activityGroups.slice((visibleActivityPage - 1) * 20, visibleActivityPage * 20);
  ...
  return <BackofficePageStack className="space-y-4">
    ...
    {errors.length ? <p role="alert">{errors.length === 2 ? c('统计暂不可用，无法判断当前运行状态。请刷新重试。', 'Statistics unavailable; runtime status is unknown. Refresh to retry.') : c('部分统计加载失败，已成功加载的数据仍可查看。点击刷新重试。', 'Some statistics failed to load. Available sources remain visible. Refresh to retry.')} ({errors.map(source => source === 'runtime' ? c('运行统计', 'Runtime') : c('插件上报', 'Plugin reports')).join('、')})</p> : null}
    ...
    <section data-ui="usage-more-reference" hidden={analysisView !== 'chart'}>
      ...
      {trend?.length ? <AnalyticsLineChart data={trend} height={220} primarySeriesName={c('运行次数', 'Runs')} secondarySeriesName={c('失败数', 'Failures')} /> : <p>{loading ? c('加载中', 'Loading') : errors.includes('runtime') ? c('运行统计暂不可用', 'Runtime statistics unavailable') : c('暂无趋势数据', 'No trend data')}</p>}
