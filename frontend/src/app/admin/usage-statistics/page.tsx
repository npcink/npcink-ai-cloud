...
export default function UsageStatisticsPage() {
  ...
  return <BackofficePageStack className="space-y-4">
    ...
    <div className={analysisView === 'table' && !pluginMode ? undefined : 'hidden'}><AdminDataTableFrame title={c('使用明细', 'Usage breakdown')} resultLabel={`${displayRows.length} ${c('组', 'groups')}`} dataUi="usage-breakdown">
    ...
    <section data-ui="usage-related-observability" className="max-w-[var(--admin-workbench-compact-max-width)] border-t border-slate-200 pt-3"><div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-sm font-semibold">{c('相关观测', 'Related observability')}</h2><p className="mt-1 text-xs text-slate-500">{c('编辑质量、媒体和向量运行证据。', 'Editorial quality, media, and vector runtime evidence.')}</p><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs"><Link className="text-blue-700 underline" href={`/admin/media-observability?window=${hours}`}>{c('媒体观测', 'Media observability')}</Link><Link className="text-blue-700 underline" href={`/admin/vector-observability?window=${hours}`}>{c('向量观测', 'Vector observability')}</Link></div></div><div><h3 className="text-sm font-semibold">{c('编辑效果', 'Editorial outcomes')}</h3>{quality ? <p className="mt-1 text-xs text-slate-500">{c('仅用于编辑辅助质量核查。', 'For editorial quality review only.')}</p> : null}<button type="button" className="btn btn-secondary btn-sm mt-2" aria-expanded={quality} onClick={() => setQuality(v => !v)}>{quality ? c('收起编辑效果', 'Hide editorial outcomes') : c('查看编辑质量证据', 'View editorial quality evidence')}</button></div></div>{quality ? <div className="mt-3"><EditorAssistQualityPanel disclosure={false} windowHours={hours} refreshSignal={refresh} /></div> : null}</section>
