...
export default function AbilityModelsPage() {
  ...
  const activeAudioPreviewResult = audioPreviewJob?.result?.audios?.[0] || null;
  const activeAudioPreviewArtifactId = activeAudioPreviewResult?.artifact?.artifact_id
    || activeAudioPreviewResult?.artifact_id
    || '';
  ...
  const activeAudioPreviewStatusLabel = audioPreviewJob?.status === 'succeeded'
    ? text('audio_preview_status_succeeded', 'Generation complete')
    : audioPreviewJob?.status === 'failed'
      ? text('audio_preview_status_failed', 'Generation failed')
                    ...
                    {activeInspectorTab === 'policy' ? (
                      ...
                      <div className="space-y-3">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
                              {text('audio_preview_title_panel', 'Audio generation check')}
                            </div>
                            <p className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-400">
                              {text('audio_preview_desc', 'Generate a short sample with the selected primary model and inspect its run metadata. Generated bytes are not exposed on this surface. This does not save the route or write to WordPress.')}
                            </p>
                          </div>
                        ...
                        {audioPreviewJob?.status === 'succeeded' ? (
                          <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs leading-5 text-slate-600 dark:border-slate-800 dark:bg-slate-900/40 dark:text-slate-300" role="status">
                            {text('audio_preview_metadata_only', 'Audio generation completed. This surface shows run and artifact metadata only; generated bytes remain behind the unified artifact delivery boundary.')}
                          </div>
                          ...
                          <div className="rounded-lg border border-dashed border-slate-300 px-3 py-2 text-xs text-slate-500 dark:border-slate-700 dark:text-slate-400">
                            {text('audio_preview_waiting', 'Audio generation is running. Run evidence will update when it completes.')}
                          </div>
                        ) : null}
                        <label className="grid gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300">
                          {text('audio_preview_text_label', 'Sample text')}
                          <textarea
                        ...
                        >
                          {audioPreviewCreating ? text('audio_preview_creating', 'Generating sample...') : text('audio_preview_action', 'Generate sample')}
                        </button>
                        ...
                        {audioPreviewJob ? (
                          <details className="rounded-lg border border-slate-200 px-3 py-2 text-xs leading-5 text-slate-500 dark:border-slate-800 dark:text-slate-400">
                            ...
                            {activeAudioPreviewResult?.mime_type ? (
                              <div className="mt-1 break-all">
                                {text('audio_preview_media_type', 'Media type')}: {activeAudioPreviewResult.mime_type}
                              </div>
                            ) : null}
                            {typeof activeAudioPreviewResult?.duration_seconds === 'number' ? (
                              <div className="mt-1 break-all">
                                {text('audio_preview_duration', 'Duration')}: {activeAudioPreviewResult.duration_seconds}s
                              </div>
                            ) : null}
                            {activeAudioPreviewArtifactId ? (
                              <div className="mt-1 break-all">
                                {text('audio_preview_artifact', 'Artifact')}: {activeAudioPreviewArtifactId}
                              </div>
                            ) : null}
                          </details>
                        ) : null}
