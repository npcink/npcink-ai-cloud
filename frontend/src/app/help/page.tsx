    ...
    >
      ...
      <DocumentSection title={zh ? '常见问题' : 'Frequently asked questions'}>
        ...
        <div>
          ...
          <p>{zh ? '可以，但必须先由原账户在 Cloud 中移除站点。同一账户可以随时重新连接；其他账户只能在站点显示的冷却期结束后、且跨账户重连开放时，通过已验证的 Addon 完成连接。套餐和额度不会随站点转移。' : 'Yes, but the previous account must remove the site in Cloud first. The same account may reconnect at any time; another account can connect through a verified Addon only after the site cooldown ends and cross-account relinking is available. Plans and credits do not move with the site.'}</p>
        </div>
      </DocumentSection>
