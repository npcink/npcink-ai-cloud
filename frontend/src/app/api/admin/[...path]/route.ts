...
const ADMIN_ROUTE_RULES: readonly AdminRouteRule[] = [
  ...
  {
    methods: ['GET'],
    pattern: /^(?:overview|plugin-observability(?:\/history)?|media-observability|vector-observability|agent-feedback|editor-assist-quality|runtime-telemetry(?:\/runs)?|agent-workflow-metadata)$/,
    namespace: 'admin',
  ...
  {
    methods: ['POST'],
    pattern: /^provider-connections\/[^/]+\/approve-image-host$/,
    namespace: 'admin',
    requiredCapability: 'can_manage_catalog',
  },
  {
  ...
  {
    methods: ['PATCH'],
    pattern: /^service-settings\/(?:portal-public|qq-login|email|alipay-payment|site-relink-policy|accounting-fx|platform-preferences|media-recognition-policy)$/,
    namespace: 'admin',
  },
  {
    methods: ['POST'],
