...
状态：开发经验与后续交接已固化；当前开发阶段和生产发布均未由本文宣告完成。
...
- `origin/master` 为
  `6f98f596c591af2d7d5c957b9a3ae9bfd5e16f05`，对应 PR `#405`；
- `origin/production` 为
  `74e074245345b1c9476e5671a762ceefee90e662`；
- 两条分支有意分叉，不能用提交数差异代替精确 tree、bundle 或部署判断；
- PR `#397` 至 `#405` 均已合并且 required checks 通过；
- PR `#405` 曾完成 clean-current-`master` M4 accepted promotion；
- 随后的 Media 语义检索批次仍在开发和真实链路验证中，因此本记录不把当前
  M4 状态、整个开发阶段或 Media 批次写成已完成；
...
该经验来自当前仍在进行的 Media 批次调查；本文只固化方法，不宣告该未合并
改动完成。
...
当前先完成 Media 及已承诺的本阶段开发任务。达到第 9.3 节的阶段关闭条件后，
以 Issue `#406` 为生产交接记录：
