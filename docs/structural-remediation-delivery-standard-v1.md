...
全仓盘点用于发现候选，不自动产生全仓重构项目。结构整改主要降低后续修改、评审和
回归成本；没有性能测量或用户路径证据时，不宣称它会提高运行性能或用户体验。

启动一个批次前，记录以下四项；允许未知，不得用估计冒充测量：

1. 受影响的具体需求、调用方或故障，以及最近一次可复核证据；
2. 当前成本：重复修改、回归、测试隔离困难或评审耗时；
3. 拆分成本：行为 characterization、调用方迁移、CI/M4 和回滚负担；
4. 完成后的可观察改善，以及何时停止继续拆分。

没有证据证明热点正在造成交付摩擦时，保留清单，随相关功能修改再评估。若需求触发
拆分，应先固定当前行为，以独立结构批次完成必要移动，再实现业务变化。

| 候选 | 默认决策 |
| --- | --- |
| 多领域全仓同时拆分 | 不作为默认项目；先证明一个热点的收益 |
| 涉及计费、队列、锁和失败恢复的核心服务 | 潜在收益高，验证成本也高；由具体需求或缺陷触发 |
| 当前需求涉及的独立纯读查询或投影 | 优先评估小批次，保留权限、错误和返回合同 |
| 已无业务分支的薄 facade | 按第 2.2 节暂停，不以删除最后一层为目标 |
| 翻译数据、声明式配置、仅因行数大的测试 | 不按行数机械拆分；按维护问题与能力边界判断 |

“适合先拆”只表示候选之间的相对顺序，不代表该重构比当前产品验证更值得立即投入。
模块长度下降不能证明职责独立：检查 mixin 的隐式依赖、跨界调用，以及页面是否仍持有
多个不相关交互的状态和请求。只搬文件或 JSX 不算 ownership 收口。

复盘时比较同类任务的修改范围、重复业务规则、回归情况和反馈耗时；不设未经测量的
收益百分比，也不以拆出文件数量作为验收指标。
